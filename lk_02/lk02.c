...
xs *xs_new_N(xs *x, void *p, const int __sizeof__)
{
    *x = xs_literal_empty();
    size_t len = strlen(p) + 1;
    _Static_assert( sizeof(*p) <= 16, "it is too big");
    if (len > AAA) {
        x->capacity = ilog2(len) + 1;
        x->size = len - 1;
        x->is_ptr = true;
...
int main()
{
    // xs string = *xs_tmp(str_foobar);
    printf("sozeof=%ld, strlen=%ld\n",sizeof(str_foobar), strlen(str_foobar));
    _Static_assert(sizeof(str_foobar) <= 16, "it is too big");
    xs string = *xs_new(&string, str_foobar);
    ...
    for(uint32_t i = 16; i < 30; i++)
    {
        printf("... <%d> ilog2=%d, __builtin_clz(i)=%d\n", i, ilog2(i), __builtin_clz(i));
    }
